    ...
    public function __construct(){
        ...
        if ($_GET['act'] !== 'seller_login') {
            ...
            if($_GET['act'] == 'seller_center') {
            	...
            	//自定义店铺模板设置时坐标menu为店铺设置
            	if(stripos($_REQUEST['act'],"store_theme_setting_")===0){
            		$left_menu = $_SESSION['seller_menu']['store']['child'];
            	}else{
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'store' => array('name' => '店铺', 'child' => array(
				...
				array('name' => '手机端设置', 'act'=>'store_mobile', 'op'=>'index'),
            )),
