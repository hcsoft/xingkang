...
class store_mobileControl extends BaseSellerControl {
	...
	public function indexOp(){
		...
		if (chksubmit()){
            ...
            $model_class->editStore($param, array('store_id' => $store_id));
            showDialog(Language::get('nc_common_save_succ'),'index.php?act=store_mobile&op=store_setting','succ');
        }
        /**
         * 实例化店铺等级模型
         */			
	...
	public function themeOp(){
		 ...
		 * 当前店铺预览图片
		 */
		$curr_image = SHOP_TEMPLATES_URL.'/store/'.$curr_store_theme.'/images/preview.jpg';
		$curr_theme = array(
		...
		foreach ($style_data as $key => $val){
			if (in_array($key,$themes)){
				...
				'image'=>SHOP_TEMPLATES_URL.'/store/'.$key.'/images/preview.jpg'
				);
			}
		}
		/**
	...
	public function set_themeOp(){
		...
		if (!empty($style) && file_exists(BASE_TPL_PATH.DS.'/store/'.$style.'/images/preview.jpg')){
			$store_class = Model('store');
	...
	private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            1=>array('menu_key'=>'store_setting','menu_name'=>'商家页面设置','menu_url'=>'index.php?act=store_mobile&op=store_setting'),
            4=>array('menu_key'=>'store_slide','menu_name'=>'商家页面广告设置','menu_url'=>'index.php?act=store_mobile&op=store_slide'),
            5=>array('menu_key'=>'store_theme','menu_name'=>'商家页面分类设置','menu_url'=>'index.php?act=store_mobile&op=theme'),
        );
