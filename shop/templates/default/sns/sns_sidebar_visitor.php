<?php defined('InShopNC') or exit('Access Invalid!');?>
  <div class="visitors pngFix">
    ...
    <ul id="visit_me" nc_type="visitlist">
      ...
      <li>
        <div class="visitor-pic"><span class="thumb size60"><i></i><a href="index.php?act=member_snshome&mid=<?php echo $v['v_mid'];?>" target="_blank"> <img src="<?php if ($v['v_mavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['v_mavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" data-param="{'id':<?php echo $v['v_mid'];?>}" nctype="mcard" onload="javascript:DrawImage(this,60,60);"> </a></span></div>
    ...
    <ul id="visit_other" nc_type="visitlist" style="display: none;">
      ...
      <li>
        <div class="visitor-pic"><span class="thumb size60"><i></i><a href="index.php?act=member_snshome&mid=<?php echo $v['v_ownermid'];?>" target="_blank"> <img src="<?php if ($v['v_ownermavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['v_ownermavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" data-param="{'id':<?php echo $v['v_ownermid'];?>}" nctype="mcard" onload="javascript:DrawImage(this,60,60);"> </a></span></div>
