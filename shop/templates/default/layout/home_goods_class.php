      ...
      <div class="category">
        <ul class="menu">
          ...
          <li cat_id="<?php echo $val['gc_id'];?>" class="<?php echo $i%2==1 ? 'odd':'even';?>" <?php if($i>8){?>style="display:none;"<?php }?>>
            ...
            <!-- -->	
            <div class="sub-class" cat_menu_id="<?php echo $val['gc_id'];?>" style="<?php if(count($val['class2'])==1)echo'width:320px;';?>">
              <?php if (!empty($val['class2']) && is_array($val['class2'])) { ?>
              <?php foreach ($val['class2'] as $k => $v) { ?>
              <dl>
              
                <dt style="<?php if(count($val['class2'])==1)echo'width:300px;';?>">
                  <h3><a href="<?php echo urlShop('search','index',array('cate_id'=> $v['gc_id']));?>"><?php echo $v['gc_name'];?></a></h3>
                </dt>
                <dd class="goods-class" style="<?php if(count($val['class2'])==1)echo'width:320px;';?>">
                  ...
                  <a href="<?php echo urlShop('search','index',array('cate_id'=> $v3['gc_id']));?>"
                  	 style="background:url();margin: 2px 2px 6px 0;padding-right: 12px;width: 62px;"><?php echo $v3['gc_name'];?></a>
                  <?php } ?>
                  <?php } ?>
                </dd>
