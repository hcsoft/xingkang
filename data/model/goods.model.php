...
class goodsModel extends Model{
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0) {
        ...
        $condition = $this->_getRecursiveClass($condition);
        if(DBDRIVER=='mssql'){
            $field = "goods_commonid+','+color_id) as nc_distinct ," . $field;
            $count = $this->getGoodsOnlineCount($condition,"distinct goods_commonid+','+color_id");
        }else{
    ...
    public function getLexusGoodsList($condition, $field = '*', $order = 'goods_id asc', $page = 0){
    	$goodslist=$this->getGoodsListByColorDistinct($condition, $field, $order, $page);
    	
    	foreach ($goodslist as $i=> &$good){
    		// 商品多图
    		$image_more = $this->getGoodsImageList(array('goods_commonid' => $good['goods_commonid'], 
    													 'color_id' => $good['color_id']), 'goods_image');
    		if(count($image_more)>=2){
    			$good['img1']=cthumb($image_more[0]['goods_image'], 240, $goods_info['store_id']);
    			$good['img2']=cthumb($image_more[1]['goods_image'], 240, $goods_info['store_id']);
    		}else{
    			$good['img1']=cthumb($good['goods_image'], 240, $goods_info['store_id']);
    			$good['img2']=cthumb($good['goods_image'], 240, $goods_info['store_id']);
    		}
    	}
