...
class stat_storeControl extends SystemControl{
    ...
    public function newstoreOp(){
		...
		if($_REQUEST['search_type'] == 'day'){
			...
			$memberlist = $model->getNewStoreStatList($where, $field, 0, '', 0, 'DAY(FROM_UNIXTIME(store_time)),HOUR(FROM_UNIXTIME(store_time))');
			if($memberlist){
		...
		if($_REQUEST['search_type'] == 'month'){
			...
			$memberlist = $model->getNewStoreStatList($where, $field, 0, '', 0, 'MONTH(FROM_UNIXTIME(store_time)),day(FROM_UNIXTIME(store_time))');
		    if($memberlist){
				foreach($memberlist as $k=>$v){
