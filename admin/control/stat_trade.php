...
class stat_tradeControl extends SystemControl{
    ...
    public function predepositOp(){
		...
		if($_REQUEST['search_type'] == 'day'){
			...
			$memberlist = $model->getPredepositInfo($where, $field, 0, '', 0, 'DAY(FROM_UNIXTIME(lg_add_time)),HOUR(FROM_UNIXTIME(lg_add_time))');
			if($memberlist){
		...
		if($_REQUEST['search_type'] == 'month'){
			...
			$memberlist = $model->getPredepositInfo($where, $field, 0, '', 0, 'MONTH(FROM_UNIXTIME(lg_add_time)) ,day(FROM_UNIXTIME(lg_add_time))');
		    if($memberlist){
				foreach($memberlist as $k=>$v){
    ...
    public function saleOp(){
		...
		if($_REQUEST['search_type'] == 'day'){
			...
			$memberlist = $model->getStoreSaleStatList($where, $field, 0, '', 0, 'DAY(FROM_UNIXTIME(add_time)),HOUR(FROM_UNIXTIME(add_time))');
			if($memberlist){
				foreach($memberlist as $k => $v){
					if($today_day == $v['dayval']){
		...
		if($_REQUEST['search_type'] == 'month'){
			...
			$memberlist = $model->getStoreSaleStatList($where, $field, 0, '', 0, 'MONTH(FROM_UNIXTIME(add_time)),day(FROM_UNIXTIME(add_time))');
		    if($memberlist){
				foreach($memberlist as $k=>$v){
					if ($up_month == $v['monthval']){
