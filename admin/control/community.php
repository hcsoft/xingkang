...
//        $page->setStyle(5);
        ...
        $startnum = $page->getEachNum() * ($page->getNowPage() - 1);
        $endnum = $page->getEachNum() * ($page->getNowPage());
        $sql = 'from Center_CheckOut a  left join Center_ClinicLog log on a.sLogKey = log.ClinicKey  and a.orgid = log.orgid, Center_codes ico
        ,
            Center_codes state,
             Organization org
...
//        Tpl::output('displaytext', $displaytext);
    ...
    public function incomesumOp()
    {
        ...
        $page->setEachNum(10);
        $page->setNowPage($_REQUEST["curpage"]);
        $sql = 'from Center_CheckOut a  , Center_codes ico,Center_codes state,
            Organization org
          where a.iCO_Type = ico.code and ico.type=\'iCO_Type\'
           and  a.iCO_State = state.code and state.type=\'iCO_State\'
           and a.orgid = org.id ';
